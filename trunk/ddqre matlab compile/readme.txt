 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_ddqre.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* ddqre.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* ddqre.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* ddqre (application created by MATLAB Compiler)
		* run_ddqre.sh (shell script that can be run 
	...
	All Platforms:            
		* ddqre.dll (shared library, file extension 
		  varies by platform)
		* ddqre.h (library header file)
		* ddqre.ctf (Component Technology File archive,
	...
	* ddqre.xml (documentation files)
	* ddqre.pdb (program database file - if 
	  DEBUG option selected)
	* ddqre.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* ddqre.jar (Java class files and compiled M-code)
	...
	* ddqre.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* ddqre.xla (Optionally created and placed in
